    ...
    public static String getHeightlightWord(String textWord, String key){
        ...
        if(!upperWord.contains(upperKey)){
            ...
            for(Map<Integer, String> map : matchList){
                thisKey = getKey(map);
                sb.replace(thisKey+keys, thisKey + +keyLen+keys, "<b><font color=blue style=\"background:yellow\">"+map.get(thisKey)+"</font></b>");
               // System.out.println(thisKey+ "/"+keyLen+map.get(thisKey)+"/"+keys);
                keys += "<b><font color=blue style=\"background:yellow\"></font></b>".length();
            }
        }
        return sb.toString();
    }
